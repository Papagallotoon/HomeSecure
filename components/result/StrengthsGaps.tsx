...
// des signaux d'état, pas des couleurs de marque. Ils restent volontairement
// désaturés pour ne pas concurrencer le bronze.
const OK = "#7fb069";
const ALERT = "#d4763a";
...
export function StrengthsGaps({
  ...
  return (
    <div className="mt-5 grid gap-5 sm:grid-cols-2">
      <div className="border border-white/[0.1] bg-brand-100/60 p-6">
        <h3 className="font-mono text-[11px] uppercase tracking-ops" style={{ color: OK }}>
          {strengthsTitle}
        </h3>
        <ul className="mt-4 flex flex-col gap-2.5">
          {strengths.length === 0 && (
            <li className="text-[15px] text-brand-400">
              None confirmed yet — start with power and water.
            </li>
          )}
          {strengths.map((s) => (
      ...
      <div className="border border-brand-600/25 bg-brand-100/60 p-6">
        <h3 className="font-mono text-[11px] uppercase tracking-ops" style={{ color: ALERT }}>
          {gapsTitle}
        </h3>
        <ul className="mt-4 flex flex-col gap-2.5">
          {gaps.length === 0 && (
            <li className="text-[15px] text-brand-400">No critical gaps detected.</li>
          )}
          {gaps.map((g) => (
